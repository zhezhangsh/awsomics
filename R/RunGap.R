...
RunGap<-function(x, func=c('kmeans', 'pam', 'hclust'), k.max=ncol(x)-1, make.plot=FALSE, 
  ...
  if (make.plot) {
    ...
    gplots::barplot2(gap[, 3], plot.ci=TRUE, ci.l=gap[,3]-gap[,4], ci.u=gap[,3]+gap[,4], space=0, ylim=c(min(gap[,3]-gap[,4])-0.05, max(gap[, 3]+gap[,4])+0.1), cex.lab=1.5, cex.sub=1.5, ylab='gap statistic', sub='Number of clusters k');
