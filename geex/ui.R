...
shinyUI(
  navbarPage(
    ...
    ################################ "Home" tab ################################ 
    tabPanel(
      "Home", htmlOutput('home.title'), br(),
      wellPanel(h4("Select to load a data collection"), selectInput("select.collection", NULL, c('', select.collection), width='200px')), 
    ...
    navbarMenu(
      ...
      tabPanel(
        ...
        wellPanel(fluidRow(
          ...
          column(3, wellPanel(
            ...
            checkboxGroupInput('expr.desc', h5('-Descriptive statistics'), choices=GetRowStatTypes(), selected=GetRowStatTypes(), inline=TRUE),
            checkboxGroupInput('expr.anno', h5('-Gene annotation'), choices=expr_anno_columns, selected=expr_anno_columns[c(1, length(expr_anno_columns))], inline=TRUE),
            br(), checkboxInput('expr.select.all', HTML(geex.html.msg('Select all')), FALSE))),
          column(6, wellPanel(
            fluidRow(
              column(6, h5('Select gene subset(s)')),
              column(6, actionButton("expr.gs.clear", 'Clear selection'))
            ), 
            fluidRow(
              column(4, selectizeInput("expr.gs.source", "Source", names(geneset), selected=names(geneset)[1])),
              column(4, selectizeInput("expr.gs.coll", "Collection", names(geneset[[1]]), selected=names(geneset[[1]])[1])),
              column(4, selectizeInput("expr.gs.species", "Species", names(geneset[[1]][[1]]), selected=names(geneset[[1]][[1]])[1])),
      ...
      tabPanel(
        ...
        wellPanel(
          ...
          fluidRow(column(2, checkboxInput('gene.filter', HTML(geex.html.msg('Filter by gene set')), value=FALSE)), column(10, uiOutput("gene.ui"))),
      ...
      tabPanel(
        ...
        wellPanel(
          fluidRow(column(2, checkboxInput('comb.filter', HTML(geex.html.msg('Filter by gene set')), value=FALSE)), column(10, uiOutput("comb.ui"))), br(),
          fluidRow( 
            column(4, wellPanel(
              fluidRow(column(5, h5('Select species')), column(6, selectizeInput("comb.species", NULL, choices='human'))),
              fluidRow(column(5, h5('Select data scale')), column(6, selectizeInput("comb.scale", NULL, choices=c('logged', 'unlogged', 'percentile'), selected='percentile'))))),
            column(4, wellPanel(
              checkboxGroupInput('comb.desc', 'Select descriptive statistics', choices=GetRowStatTypes(), selected=GetRowStatTypes()[1:(length(GetRowStatTypes())-1)], inline=TRUE))),
            column(4, wellPanel(
              checkboxGroupInput('comb.anno', 'Select gene annotation', choices=expr_anno_columns, selected=expr_anno_columns[1], inline=TRUE))))),
                 ...
                 wellPanel(
                   fluidRow(
                     ...
                     column(6, uiOutput("lookup.table.ui")
                            #DT::dataTableOutput('lookup.table'))
                     ))))
      ...
      tabPanel(
        ...
        fluidRow(
          column(8, wellPanel( fluidRow(
            column(4, wellPanel( 
              h4("Select plot"),
              fluidRow(column(4, h5("Plot\ntype")), column(8, selectizeInput("geneset.type", NULL, PlotMatrixGroupTypes()))),             
              fluidRow(column(4, h5("Data\nscale")), column(8, selectizeInput("geneset.scale", NULL, c('logged', 'unlogged', 'percentile')))),
              fluidRow(column(4, h5("Color\nrange")), column(8, selectizeInput("geneset.color", NULL, GetColorTypes()))))),    
            column(4, wellPanel( 
              h4("Select data set"), 
            ...
            column(4, wellPanel( h4('Select group(s)'), checkboxGroupInput("geneset.group", NULL, label=, choices=c())))), 
          ...
          column(4,
                 wellPanel(
                   ...
                   column(3, h5("Source")), 
                   column(8, selectizeInput("geneset.source", NULL, names(geneset), selected=names(geneset)[1], options=list('caption-side'='bottom'))),
                   column(3, h5("Collection")), 
                   column(8, selectizeInput("geneset.coll", NULL, names(geneset[[1]]), selected=names(geneset[[1]])[1])),
                   column(3, h5("Species")), 
                   column(8, selectizeInput("geneset.species", NULL, names(geneset[[1]][[1]]), selected=names(geneset[[1]][[1]])[1])),
      ...
      tabPanel(
        ...
        fluidRow(
          column(8, wellPanel( fluidRow(
            column(4, wellPanel( 
              h4("Select plot"),
              fluidRow(column(4, h5("Plot\ntype")), column(8, selectizeInput("two.type", NULL, PlotPairDiffTypes()))),             
              fluidRow(column(4, h5("Data\nscale")), column(8, selectizeInput("two.scale", NULL, c('logged', 'unlogged', 'percentile')))),
              fluidRow(column(4, h5("Color\nrange")), column(8, selectizeInput("two.color", NULL, GetColorTypes()))),    
              fluidRow(column(4, h5("Use\nspecies")), column(8, selectizeInput("two.select.species", NULL, c()))))),
            column(4, wellPanel(
              h4('Select X-axis'),
